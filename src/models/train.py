...
def train_3day_forecaster():
    try:
        ...
        # 3. Time-based split with consistent indices
        split_idx = int(0.8 * len(features))
        X_train, X_test = features.iloc[:split_idx].copy(), features.iloc[split_idx:].copy()
        y_train, y_test = targets.iloc[:split_idx].copy(), targets.iloc[split_idx:].copy()
        
        # 4. Explicit index alignment
        ...
        # 5. Verify alignment
        print("\n=== Index Verification ===")
        print("X_train index range:", X_train.index[0], "to", X_train.index[-1])
        print("y_train index range:", y_train.index[0], "to", y_train.index[-1])
        assert len(X_train) == len(y_train), f"Length mismatch: X_train {len(X_train)} vs y_train {len(y_train)}"
        ...
        # 7. Train final models (one per horizon)
        ...
        # 9. Generate predictions and evaluate
        validation_results = []
        for h_name, h_idx in horizon_map.items():
            try:
